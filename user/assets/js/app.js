...
db.version(1).stores({
  ...
  banner: "id,img,type",
  cart: "id,var_id,product_id,product_nm,cat_id,cat_nm,imgs,price,sku,purchase_qty",
  ...
  return `
        <div class="product-cart-div product-cart-div2">
            ...
            <div class="product-details-cart detail-cart">
                <div class="product-details-cart-left-div" onclick="gotoProduct(${
                  item.id
                })">
                    <img src="${imgLink + item.imgs[0]}" alt="Product 1">
                </div>
                <div class="product-details-cart-right-div more-div-update">
                    <h3 onclick="gotoProduct(${item.id})">${
    item.product_nm
  }</h3>
                <span class="span-class">  
                    ...
                    <del>${fprice}</del>  
                    <p id="total${item.id}">Total: Rs ${
    item.price * item.purchase_qty
  }</p>
                    </span>
                    <div class="qty qty-counter">
                    ...
                    <div class="updated-class1">
                        ...
                        <input type="number" class="count update-btn-wala" name="pr_qty${
                          item.id
                        ...
                        <span 
                            ...
                            <button class="remove-btn" onclick="removeSingleProductFromCart(${
                              item.id
...
function filterProduct(inp, data = 0) {
  ...
  filterProductProcess(srchValue, filterValue);
}
            ...
            <button class="minus" onclick='decreaseQty(${JSON.stringify(
              item
                ...
                <input type="text" class="pr_${item.id}"
                    value="${
                      item.purchase_qty == undefined ? 0 : item.purchase_qty
                    }" 
                    class="pr${item.id}">
    ...
    return `
            <div class="navbar">
                <div class="nav1">
                    ...
                    <div class="img-nav">
                        <span>${
                          name.length != 0 ? name.slice(0, 1) : "G"
                        }</span>
                    </div>
                </div>
            </div>
...
function productImageSlider(data) {
  function productdetaildata() {
    const maindata = data
      .map((item) => `<img src="${item.img}" alt="image">`)
      .join("");
...
async function productCartCreator(product_limit, sort_by) {
  const [products, wishlistData, cartItem] = await Promise.all([
    db.products.limit(product_limit).toArray(),
    db.wishlist.toArray(),
    db.cart.toArray(),
  ]);
...
async function loadBanner() {
    ...
    bannerListOneHTML += `
            <a href="#">
              <div class="banner banner-big">
                <img src="${imgLink + topBanner1[0].img}" alt="${
      topBanner1[0].id
    }" />
              </div>
            </a>`;
  ...
  topBanner2.forEach((item) => (bannerListOneHTML += bannerCardTemplate(item)));
  ...
  return `
        <div class="wishlist-items">
            <div class="wishlist-item-img">
                <i class="bi bi-heart-fill" onclick="removeSingleProductFromWL(${
                  item.id
                })"></i>
                <img 
                    src="${imgLink + item.imgs[0]}" 
